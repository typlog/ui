...
export function markdownExampleTagBlock(md: MarkdownRenderer) {
  md.core.ruler.after('inline', 'example', (state) => {
    ...
    state.src = state.src.replace(regex, (content, bindingValue) => {
      ...
      const { path: mdPath } = state.env as unknown as MarkdownEnv
      ...
      srcToken.info = 'vue'
      srcToken.content = source
        .replace(/#components/g, '@typlog/ui')
        .replace(/#addons/g, '@typlog/ui/addons')
        .replace(/#charts/g, '@typlog/ui/charts')
        .trim()
